...
it('should trigger only the change type event', () => {
  ...
  head.find('select').first().simulate('change', { target: { value: "glass" }});

  expect(props.onFilter.mock.calls[0]).toBeUndefined()
  expect(props.onFetch.mock.calls[0]).toBeUndefined()
  expect(props.onChangeType.mock.calls[0][0]).toEqual({ target: { value: "glass" }})
});
...
it('should trigger only the filter event with text', () => {
  ...
  head.find('input[type="text"]').first().simulate('change', { target: { value: "bulbasaur" }});
...
it('should trigger only the fetch event with number', () => {
  ...
  head.find('input[type="number"]').first().simulate('change', { target: { value: "133" }});
...
it('should not trigger the fetch event with no number', () => {
  ...
  head.find('input[type="number"]').first().simulate('change', { target: { value: "asd" }});
