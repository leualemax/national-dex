...
it("renders without crashing", () => {
  ...
  expect(pokemon.find("h3").text()).toEqual(
    `${Missingno.id} - ${Missingno.name}`
  );
  expect(pokemon.find("img").props().src).toEqual(
    Missingno.sprites.front_default
  );
...
it("renders with description", () => {
  ...
  expect(pokemon.find(".description").text()).toEqual(
    "日なたで　昼寝を　する　姿を　見かける。↵太陽の　光を"
  );
});
