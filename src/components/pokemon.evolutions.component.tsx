...
> {
  ...
  public chain(evolution: IEvolution | IEvolution[]) {
    return Array.isArray(evolution) ? (
      evolution.map((e: IEvolution, key: number) => {
        return (
          <li key={key}>
  ...
  public evolutions(
    evolution?: IEvolution | IEvolution[]
    ...
    evolution =
      Array.isArray(evolution) && evolution.length === 0
        ? undefined
        : evolution;
    return evolution && <ul>{this.chain(evolution)}</ul>;
  }

  public render() {
    return <ul className="block evolutions list">{this.evolutions()}</ul>;
  }
}
